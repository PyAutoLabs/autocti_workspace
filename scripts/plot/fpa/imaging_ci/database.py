...
The model-fit was performed to 36 charge injection datasets consisting of 1 charge injection level across the
6x6 FPA.

We first seek to plot these 36 datasets, on a single matplotlib figure, so we can cleanly see all datasets across
the FPA on at once.

We use the `ImagingCIAgg` object to create a generator of every dataset, which we iterate over to build a list
