...
This script uses the results generated by the script `/autocti_workspace/database/tutorial_1_introduction.py`, which
...
This script covers how to manipulate the `Samples` object returned from a *PyAutoCTI* model-fit, which you have most
...
A model instance contains all the model components of our fit, for example the CTI mdoel we specified during 
model composition.
"""
...
"""
The CTI model components are named according to those input into the search (in this case, `trap_list` and `ccd``).
"""
...
"""
Their parameters can be printed.
"""
